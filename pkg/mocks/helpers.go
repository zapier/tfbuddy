...
func (ts *TestSuite) InitTestSuite() {
	...
	ts.MockGitClient.EXPECT().CreateMergeRequestDiscussion(gomock.Any(), ts.MetaData.MRIID, ts.MetaData.ProjectNameNS, &RegexMatcher{regex: regexp.MustCompile(`Starting TFC apply for Workspace: ` + "`" + `([A-z\-]){1,}/([A-z\-]){1,}` + "`" + `\.\n<!-- tfbuddy:ws=.+:action=.+ -->`)}).Return(ts.MockGitDisc, nil).AnyTimes()
